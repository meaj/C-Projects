 ...
 *      array of mutexes to represent the philosopher's chopsticks. It makes use of pthread_mutex_trylock()
...
//      If pthread_mutex_trylock() != 0, thinking() is called, otherwise the appropriate mutexes are locked
void pickUpChopsticks(int threadIndex){
    // Wait for successful mutex lock on philosopher thead's chopstick
    while(pthread_mutex_trylock(&chopsticks[threadIndex]) != 0){
...
void putDownChopsticks(int threadIndex){
    ...
    if(threadIndex != maxIndex-1){
        pthread_mutex_unlock(&chopsticks[threadIndex+1]);
    }
    else{
...
//      Prints the ID of the thread, calls pickUpChopsticks(), and phtead_exit()
void *PhilosopherThread(void *threadid){
    long philosopherID;
    philosopherID = (long)threadid;
    printf("This is philosopher %ld\n", philosopherID+1);
    pickUpChopsticks((int)philosopherID);
    pthread_exit(NULL);
}
